...
function print_function_info($url_params, $info, $sort, $run1, $run2) {
    ...
    if ($display_calls) {
        // Call Count..
        print_td_num($info["ct"], $format_cbk["ct"], ("ct" == $sort_col));
        print_td_pct($info["ct"], $totals["ct"], ("ct" == $sort_col));
    }
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
    ...
    foreach ($stats as $stat) {
        ...
        if (is_array($sortable_columns) && array_key_exists($stat, $sortable_columns)) {
            $href = "$base_path/?"
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
    ...
    print("<center><br><h3>" .
        xhprof_render_link($callgraph_report_title,
            "$base_path/callgraph" . "?" . http_build_query($url_params))
        . "</h3></center>");
...
function pc_info($info, $base_ct, $base_info, $parent) {
    ...
    if ($display_calls) {
        ...
        /* call count */
        print_td_num($info["ct"], $format_cbk["ct"], ("ct" == $sort_col), $mouseoverct);
        print_td_pct($info["ct"], $base_ct, ("ct" == $sort_col), $mouseoverct);
    }
...
function print_source_link($info) {
    if (strncmp($info['fn'], 'run_init', 8) && 'main()' !== $info['fn']) {
